...
<Box className="flex gap-4 mobile:pl-4  items-center  ">
  ...
  <TabList mb='1em' overflowX="scroll" width="full" overflowY="hidden" sx={{
                  "&::-webkit-scrollbar": {
                    height: "6px", // Change the height of the scrollbar
                  },
                  "&::-webkit-scrollbar-thumb": {
    ...
    { tenantsNames&&tenantsNames?.map((items,index)=>{
      return(
        <Tab key={index} width="100%" color="rgba(250,239,244,.8)" className=" text-white mobile:min-w-[150px] mobile:text-xs sm:text-base  sm:min-w-[200px]"   _selected={{ color: "white", bg: "green.500" }}>{items[1]}</Tab>
      )
    })  }
  </TabList>
...
>
  ...
  bgColor="rgba(36,54,88,.5)"
    ...
    alt='Caffe Latte'
  />
<Box display="flex" className="w-full mobile:flex-col mobile:text-xs md:text-xs lg:text-base sm:flex-row justify-between">
<Stack width="100%">
